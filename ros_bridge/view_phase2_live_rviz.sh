#!/usr/bin/env bash
# Run the selected Phase-1 + Phase-2 policy in Isaac and show it live in RViz.
# Usage: bash ros_bridge/view_phase2_live_rviz.sh [easy|medium|hard] [rate_hz]
set -eo pipefail
...
echo "[view-live] Phase-2 residual policy, condition=$CONDITION rate=${RATE}Hz"
echo "[view-live] Red=actual EE, green=target; Isaac runs headless."
...
OMNI_KIT_ACCEPT_EULA=YES "$ROOT/env_isaaclab/bin/python" \
    "$ROOT/scripts/live_phase2_stream.py" \
